...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        self.cursor.execute("SELECT a.id,c.id as webid,b.priceone as priceone,b.pricetwo as pricetwo,b.id as keywordid,a.`username`,b.platformid,b.name as keywordname,c.websiteurl "
                            ...
                            "ON a.id=c.`userid` "
                            "WHERE b.platformid=5 and a.balance>0 and c.id=121 "
    ...
    def parse(self, response):
        ...
        if domain_rank == -1 and root_pn != 50:
            ...
            header = {
                ...
                "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
                'User-Agent': "Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1"}
            yield scrapy.Request(url=first_url,headers=header, meta={'page_n':root_pn,'root_url':root_url,'keywordid':keywordid,'user_id':user_id,'webid':webid,'priceone':priceone,'pricetwo':pricetwo,'root_name_all':keyword,'root_user_url':root_user_url},callback=self.parse)
