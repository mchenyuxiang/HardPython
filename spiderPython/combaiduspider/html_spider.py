...
class SpiderMain(object):
    ...
    # mobile百度排名查询
    def baidu_mobile_rank_craw(self,root_url,root_user_url,root_name_all):
        for root_name_single in root_name_all:
            root_name = quote_plus(root_name_single)
            # 得到:关键词列表
            root_pn = 0
    ...
    # sougou百度排名查询
    def sougou_rank_craw(self,root_url,root_user_url,root_name_all):
        for root_name_single in root_name_all:
            root_name = quote_plus(root_name_single)
            # 得到:关键词列表
            root_pn = 1
    ...
    # 360百度排名查询
    def sou_360_rank_craw(self,root_url,root_user_url,root_name_all):
        for root_name_single in root_name_all:
            root_name = quote_plus(root_name_single)
            # 得到:关键词列表
            root_pn = 1
            ...
            for root_pn in range(1, 6 ,1):
                ...
                html_cont = self.downloader.download(html_url)
                # 得出网址
                new_data = self.parser.sou_360_paser(html_cont)
                # 得到客户的域名地址
                user_url_data = root_user_url.split(".")
                leng_url = len(user_url_data)
                if leng_url == 1:
    ...
    # sougou百度排名查询
    def sougou_mobile_rank_craw(self,root_url,root_user_url,root_name_all):
        for root_name_single in root_name_all:
            root_name = quote_plus(root_name_single)
            # 得到:关键词列表
            root_pn = 1
            ...
            for root_pn in range(1, 2 ,1):
                ...
                html_cont = self.downloader.download(html_url)
                # 得出网址
                new_data = self.parser.sougou_mobile_paser(html_cont)
                # 得到客户的域名地址
                user_url_data = root_user_url.split(".")
                leng_url = len(user_url_data)
                if leng_url == 1:
